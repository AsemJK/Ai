...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # allow all origins: Streamlit's default port ["http://localhost:8501", "*"]
...
async def rag_query_endpoint(request: RAGRequest):
    ...
    context_text = "\n\n".join([f"[Source: {r['source']}] {r['text']}" for r in context_results])
...
async def ingest_file_endpoint(
    file: UploadFile = File(..., description="The document to ingest (PDF, DOCX, XLSX)"),
    ...
    try:
        ...
        if not extracted_text.strip():
            raise HTTPException(status_code=400, detail="Extracted text is empty. Is the file corrupted or image-based?")
